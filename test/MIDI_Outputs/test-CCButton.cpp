...
TEST(CCButton, pressAndRelease) {
    ...
    EXPECT_CALL(midi, sendImpl(0xB6, 0x3C, 0x7F, 0xC));
    button.update();
    ...
    EXPECT_CALL(midi, sendImpl(0xB6, 0x3C, 0x00, 0xC));
    button.update();
...
TEST(CCButtonBankable, changeSettingAndPressAndRelease) {
    ...
    EXPECT_CALL(ArduinoMock::getInstance(), digitalRead(2))
        .WillOnce(Return(LOW));
    EXPECT_CALL(ArduinoMock::getInstance(), millis()).WillOnce(Return(2000));
    EXPECT_CALL(midi, sendImpl(0xB6, 0x3C + 4, 0x7F, 0xC));
    button.update();
    ...
    EXPECT_CALL(ArduinoMock::getInstance(), digitalRead(2))
        .WillOnce(Return(HIGH));
    EXPECT_CALL(ArduinoMock::getInstance(), millis()).WillOnce(Return(4000));
    EXPECT_CALL(midi, sendImpl(0xB6, 0x3C + 4, 0x00, 0xC));
    button.update();
...
TEST(CCButtons, pressAndRelease) {
    ...
    EXPECT_CALL(midi, sendImpl(0xB6 + 2, 0x3C + 1, 0x7F, 0xC));
    buttons.update();
    ...
    EXPECT_CALL(midi, sendImpl(0xB6 + 2, 0x3C + 1, 0x00, 0xC));
    buttons.update();
...
TEST(CCButtonsBankable, pressChangeBankAndReleaseAndPress) {
    ...
    EXPECT_CALL(midi, sendImpl(0xB6 + 2, 0x3C + 1 + 4, 0x7F, 0xC));
    buttons.update();
    ...
    EXPECT_CALL(midi, sendImpl(0xB6 + 2, 0x3C + 1 + 4, 0x00, 0xC));
    buttons.update();
