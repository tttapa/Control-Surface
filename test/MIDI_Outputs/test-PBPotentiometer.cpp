...
TEST(PBPotentiometer, simple) {
    ...
    InSequence s;
    EXPECT_CALL(midi, sendImpl(0xE6, low(16), high(16), CABLE_13));
    pot.update();
    EXPECT_CALL(midi, sendImpl(0xE6, low(28), high(28), CABLE_13));
    pot.update();
    EXPECT_CALL(midi, sendImpl(0xE6, low(37), high(37), CABLE_13));
    pot.update();
...
TEST(PBPotentiometer, mapping) {
    ...
    EXPECT_CALL(ArduinoMock::getInstance(), analogRead(2))
        ...
        .WillRepeatedly(Return(512));
    InSequence s;
    EXPECT_CALL(midi, sendImpl(0xE6, low(16 * 2), high(16 * 2), CABLE_13));
    pot.update();
    EXPECT_CALL(midi, sendImpl(0xE6, low(28 * 2), high(28 * 2), CABLE_13));
    pot.update();
    EXPECT_CALL(midi, sendImpl(0xE6, low(37 * 2), high(37 * 2), CABLE_13));
    pot.update();
...
TEST(PBPotentiometer, invert) {
    ...
    InSequence s;
    EXPECT_CALL(midi,
                sendImpl(0xE6, 127 - low(16), 127 - high(16), CABLE_13));
    pot.update();
    EXPECT_CALL(midi,
                sendImpl(0xE6, 127 - low(28), 127 - high(28), CABLE_13));
    pot.update();
    EXPECT_CALL(midi,
                sendImpl(0xE6, 127 - low(37), 127 - high(37), CABLE_13));
    pot.update();
