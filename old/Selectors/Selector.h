...
class Selector {
  public:
     ...
     * @param   switchPin
     *          The pin with the switch connected.
     *          The built-in pull-up resistor will be enabled.
     * @param   switchType
     *          The type of switch. (See Selector::SwitchType)
     */
    Selector(pin_t switchPin,
             SwitchType switchType = MOMENTARY) // One switch or button, no LEDs
        : nb_settings(2) {
     ...
     * Setting 1 is selected and the LED is off
     * When the switch is in the 'on' position,
     * Setting 2 is selected and the LED is on
     *
     * @note    This mode is pretty useless, you can just connect the
     *          LED to the switch directly, without wasting a digital
     *          output pin on it.
     *
     ...
     * When the button is pressed again, Setting 1 is selected and the LED is
     * turned off,
     * and so on.
     *
     ...
     *          The type of switch. (See Selector::SwitchType)
     */
    Selector(pin_t switchPin, pin_t ledPin,
             SwitchType switchType = MOMENTARY) // One switch or button, one LED
     ...
     * @param   switchPins
     *          A list of pin numbers with the buttons connected.
     */
    template <size_t N>
    ...
    /**
     * @brief   Construct a Selector object with multiple buttons and
     *          multiple LEDs.

     * <h3>Multiple momentary switches (push buttons) and multiple LEDs</h3>
     *
     * Pressing one of the buttons selects the respective output and
     * enables the respective LED:
     * When starting the program, Setting 1 is selected and LED 1 is on,
     * When the second button is pressed, Setting 2 is selected, LED 1 turns off
     * and LED 2 turns on,
     * When the N-th button is pressed, Setting N is selected, the N-th LED
     * turns on, and all other LEDs are off.
     *
     * Selector( { buttonPin1, buttonPin2, ... , buttonPinN },
     * { ledPin1, ledPin2, ... , ledPinN } );

    @image html "selector-multiple-momentary-switches-LED.svg"

     * <h3>Two momentary switches (push buttons) and multiple LEDs
     * (increment / decrement)</h3>
     *
     * Pressing the first button increments the Setting number and turns on the
     * respective LED,
     * pressing the second button decrements the Setting number and turns on the
     * respective LED:
     * When starting the program, Setting 1 is selected and LED 1 is on,
     * When the first button is pressed, Setting 2 is selected, LED 1 turns off
     * and LED 2 turns on,
     * When the first button is pressed again, Setting 3 is selected, LED 2
     * turns off and LED 3 turns on.
     ...
     * When the second button is pressed, the last Setting (M) is selected,
     * LED 1 turns off and LED M turns on,
     * When the second button is pressed again, Setting (M-1) is selected,
     * LED M turns off and LED M-1 turns on,
     * and so on.
     *
     * Selector( { incrementButtonPin, decrementButtonPin },
     * { ledPin1, ledPin2, ... , ledPinM });

    @image html "selector-increment-decrement-LED.svg"

     * <h3>One momentary switch (push button) and multiple LEDs (increment)</h3>
     *
     * Pressing the button increments the Setting number and turns on the
     * respective LED,
     * When starting the program, Setting 1 is selected and LED 1 is on,
     * When the button is pressed, Setting 2 is selected, LED 1 turns off and
     * LED 2 turns on,
     * When the button is pressed again, Setting 3 is selected, LED 2 turns off
     * and LED 3 turns on.
    ...
    Selector(
        const pin_t (&switchPins)[N],
        const pin_t (&ledPins)[M]) // One or multiple buttons, multiple LEDs
        : nb_settings(M) {
        // TODO: can this ever happen?
        static_assert(N > 0, "Error: no switch pins specified. ");
        // TODO: can this ever happen?
        static_assert(M > 0, "Error: no LED pins specified. ");
        static_assert(N <= 2 || N == M, "Error: number of LEDs should be the "
                                        "same as the number of buttons. ");
     ...
     * When the second button is pressed, the last Setting (nb_settings) is
     * selected,
     * When the second button is pressed again, the previous Setting
     * (nb_settings-1) is selected,
     * and so on.
     *
     ...
     * @tparam  N
     *          The number of buttons [1, 2].
     * @param   switchPins
     *          A list of pin numbers with the buttons connected.
     * @param   nb_settings
     *          The number of settings to cycle through.
     */
    template <size_t N>
    Selector(const pin_t (&switchPins)[N],
             uint8_t nb_settings) // Two buttons (+1, -1), no LEDs
    ...
    Selector *getNext() { return next; }
    static Selector *getFirst() { return first; }
  ...
  protected:
    ...
    /**
     * @brief   A virtual function that gets called every time the setting
     *          changes.
     *
     * @param   newSetting The new setting that will be selected after the
