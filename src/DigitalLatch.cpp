...
void DigitalLatch::refresh(){
  ...
  if(value != oldVal){
    if(offSent){  // If the note is turned off
      ...
      if(bankTrue && !digitalRead(bankPin)){
        USBMidiController.send(NOTE_OFF, newChannel, newNote, velocity);  // Turn off the note 
      } 
      else 
      {
        USBMidiController.send(NOTE_OFF, channel, note, velocity);  // Turn off the note 
      }
      if(bankTrue && !digitalRead(bankPin)){
        USBMidiController.send(NOTE_ON, newChannel, newNote, velocity);   // Turn the note on again
      } 
      else 
      {
        USBMidiController.send(NOTE_ON, channel, note, velocity);   // Turn the note on again
      }
      time = millis();  // store the time of the note on message
    }
    oldVal = value;
