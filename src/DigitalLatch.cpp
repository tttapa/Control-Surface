...
void DigitalLatch::refresh() // Check if the button state changed, if so, send a MIDI Note On, after a non-blocking delay of "latchTime", send a Note Off
{
  ...
  if (state != oldState)         // If the switch changed position
  {
    ...
    {
      sendMIDI(NOTE_ON, channel + channelOffset, note + addressOffset, velocity); // Turn on the note
    ...
    {
      sendMIDI(NOTE_OFF, channel + channelOffset, note + addressOffset, velocity); // Turn off the note
      sendMIDI(NOTE_ON, channel + channelOffset, note + addressOffset, velocity);  // Immediately turn the note on again
      noteOnTime = millis();                                                                     // store the time of the note on message
    }
    oldState = state;
  }
