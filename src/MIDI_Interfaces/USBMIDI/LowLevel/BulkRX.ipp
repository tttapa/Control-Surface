...
void BulkRX<Derived, MessageTypeT, PacketSizeV>::reset() {
    reading.available.store(0, mo_rlx);
    reading.read_idx = 0;
    reading.write_idx.store(0, mo_rlx);
    reading.reading.store(true, mo_rlx);
    CRTP(Derived).rx_start(reading.buffers[0].buffer, PacketSize);
}

template <class Derived, class MessageTypeT, uint16_t PacketSizeV>
bool BulkRX<Derived, MessageTypeT, PacketSizeV>::read(MessageType &message) {
    ...
    static_assert(PacketSize % sizeof(MessageType) == 0, "");
    ...
    if (read_buffer.index == read_buffer.size) {
        ...
        if (reading.reading.exchange(true, mo_acq) == false) {
            ...
            CRTP(Derived).rx_start(reading.buffers[w].buffer, PacketSize);
        }
    }
...
template <class Derived, class MessageTypeT, uint16_t PacketSizeV>
void BulkRX<Derived, MessageTypeT, PacketSizeV>::rx_callback(
    ...
    uint32_t w = reading.write_idx.load(mo_rlx);
    if (num_bytes_read == 0) {
        CRTP(Derived).rx_start_isr(reading.buffers[w].buffer, PacketSize);
        return;
    }
