    ...
    while (1) {
      if (rx_packet == nullptr) // If there's no previous packet
      {
        if (!usb_configuration) // Check USB configuration
          return NO_MESSAGE;
        rx_packet =
            usb_rx(MIDI_RX_ENDPOINT); // Read a new packet from the USB buffer
        if (rx_packet == nullptr) {   // If there's no new packet, return
          return NO_MESSAGE;
        }
        if (rx_packet->len <
            4) // If the lenght is less than 4, it's not a valid MIDI USB packet
        {
      ...
      {
        rx_packet->index =
            index; // Change the index and read the next four bytes on next read
