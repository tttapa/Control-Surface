...
MCU_VPot_Ring::MCU_VPot_Ring(uint8_t track, uint8_t nb_tracks)
    : MIDI_Input_Element_CC(track + 0x30 - 1, 1, nb_tracks, 1), track(track),
      nb_tracks(nb_tracks) {
  values = new uint8_t[nb_tracks];
  memset(values, 0, sizeof(uint8_t) * nb_tracks);
}

MCU_VPot_Ring::~MCU_VPot_Ring() { delete[] values; }

bool MCU_VPot_Ring::updateImpl(const MIDI_message_matcher &midimsg) {
  uint8_t index = (midimsg.data1 - address) / tracksPerBank;
  uint8_t data2 = (midimsg.data2 & 0xF0) |
                  ((midimsg.data2 & 0x0F) < 0x0C ? midimsg.data2 : 0xB);
...
uint8_t MCU_VPot_Ring::getPosition() { return getPosition(addressOffset); }
uint8_t MCU_VPot_Ring::getCenterLED() { return getCenterLED(addressOffset); }
uint8_t MCU_VPot_Ring::getMode() { return getMode(addressOffset); }
...
MCU_VPot_Ring_LED::MCU_VPot_Ring_LED(const pin_t (&LEDs)[11], uint8_t track,
                                     uint8_t nb_tracks)
...
MCU_VPot_Ring_LED::MCU_VPot_Ring_LED(const pin_t (&LEDs)[11],
                                     pin_t centerLEDpin, uint8_t track,
                                     uint8_t nb_tracks)
    : MCU_VPot_Ring(track, nb_tracks), centerLEDpin(centerLEDpin),
      centerLED(true) {
  copy(this->LEDs, LEDs);
  for (pin_t pin = 0; pin < 11; pin++)
    ExtIO::pinMode(LEDs[pin], OUTPUT);
