...
MCU_7SegmentDisplay::~MCU_7SegmentDisplay() { free(textBuffer); }
bool MCU_7SegmentDisplay::updateImpl(const MIDI_message_matcher &midimsg) {
  ...
  DEBUGREF("\tCharacter: " << charIndex << '\t' << character
                           << (decimalPt ? '.' : ' '));
...
void MCU_TimeDisplay::getText(char *buff) { strncpy(buff, &textBuffer[0], 10); }
...
MCU_AssignmentDisplay::MCU_AssignmentDisplay()
    : MCU_7SegmentDisplay(0x4A, 0x2) {}
