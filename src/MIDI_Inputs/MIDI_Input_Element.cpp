...
MIDI_Input_Element::MIDI_Input_Element(uint8_t address, uint8_t channel,
                                       uint8_t nb_addresses,
                                       uint8_t nb_channels)
    : address(address), channel(channel - 1), nb_addresses(nb_addresses),
...
bool MIDI_Input_Element::updateWith(const MIDI_message_matcher &midimsg) {
  ...
  if (!updateImpl(midimsg))
    return false;
  DEBUGFN("Match: " << endl
                    << '\t' << "address = " << hex << this->address << endl
                    << '\t' << "channel = " << this->channel << dec);
  moveDown(); // Most frequently updated inputs sink to the bottom for
...
bool MIDI_Input_Element::matchChannel(uint8_t targetChannel) {
  ...
  return (channelDiff >= 0) && (channelDiff < nb_channels * tracksPerBank) &&
         (channelDiff % tracksPerBank == 0);
}

void MIDI_Input_Element::setChannelOffset(uint8_t offset) {
  channelOffset = offset % nb_channels;
  display();
}
void MIDI_Input_Element::setAddressOffset(uint8_t offset) {
...
MIDI_Input_Element_CC::MIDI_Input_Element_CC(uint8_t address, uint8_t channel,
                                             uint8_t nb_addresses,
                                             uint8_t nb_channels)
...
MIDI_Input_Element_CC *MIDI_Input_Element_CC::getFirst() { return first; }
MIDI_Input_Element_CC *MIDI_Input_Element_CC::getLast() { return last; }
MIDI_Input_Element_CC *MIDI_Input_Element_CC::getNext() { return next; }
...
MIDI_Input_Element_Note::MIDI_Input_Element_Note(uint8_t address,
                                                 uint8_t channel,
                                                 uint8_t nb_addresses,
                                                 uint8_t nb_channels)
    : MIDI_Input_Element(address, channel, nb_addresses, nb_channels) {
...
MIDI_Input_Element_Note *MIDI_Input_Element_Note::getFirst() { return first; }
MIDI_Input_Element_Note *MIDI_Input_Element_Note::getLast() { return last; }
MIDI_Input_Element_Note *MIDI_Input_Element_Note::getNext() { return next; }
...
MIDI_Input_Element_ChannelPressure::MIDI_Input_Element_ChannelPressure(
    uint8_t address, uint8_t channel, uint8_t nb_addresses, uint8_t nb_channels)
    : MIDI_Input_Element(address, channel, nb_addresses, nb_channels) {
  LinkedList::append(this, first, last);
}
MIDI_Input_Element_ChannelPressure::~MIDI_Input_Element_ChannelPressure() {
...
bool MIDI_Input_Element_ChannelPressure::match(
    const MIDI_message_matcher &midimsg) {
...
MIDI_Input_Element_ChannelPressure *
MIDI_Input_Element_ChannelPressure::getLast() {
  return last;
}
MIDI_Input_Element_ChannelPressure *
...
MIDI_Input_Element_ChannelPressure *MIDI_Input_Element_ChannelPressure::last =
    nullptr;
MIDI_Input_Element_ChannelPressure *MIDI_Input_Element_ChannelPressure::first =
    nullptr;
