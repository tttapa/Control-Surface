...
public:
  ...
  Transposer(Bank &bank, const T (&switchPins)[N], int8_t minimumSemitones = -12, int8_t maximumSemitones = 12) 
  : Selector((const pin_t (&)[N])switchPins, (size_t)maximumSemitones - minimumSemitones + 1),
  bank(bank), minimumSemitones(minimumSemitones) {
      setSetting(-minimumSemitones);
  }

  int8_t getTransposition() {
      return settingToSemitones(getSetting());
  }
