...
public:
  VUDisplay(Adafruit_GFX &display, MCU_VU &vu, Location loc, uint16_t width,
  ...
  void draw() {
    ...
    if (peak > 0) {
      display.drawFastHLine(
          x, y - spacing + blockheight - peak * (blockheight + spacing), width,
          color);
      for (uint8_t i = 0; i < value; i++)
        display.fillRect(x, y - i * (blockheight + spacing), width, blockheight,
                         color);
    }
  }
