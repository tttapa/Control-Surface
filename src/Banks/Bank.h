 ...
 * 
 * @f$ c = c_0 + p \cdot s @f$ &emsp; where @f$c@f$ is the effective channel, @f$c_0@f$ is the base channel of the MIDI_Element, @f$p@f$ is the number of tracks per bank, and @f$s@f$ is the current bank setting.
 */
class Bank
{
public:
 ...
 * 
 * @param tracksPerBank The number of tracks (i.e. addresses or channels) that are skipped when the bank setting changes.
 */
  ...
  enum bankType
  {
    ...
    CHANGE_CHANNEL,
    /**
     * @brief Change the offset of the address (i.e. Controller number or Note number) of the MIDI_Element.
     * 
     */
    CHANGE_ADDRESS
  };
  ...
  /**
   * @brief Apply a mapping function to all Analog elements (AnalogBase::map) in the bank.
   * 
